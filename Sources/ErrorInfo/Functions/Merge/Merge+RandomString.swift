...
extension Merge.Constants {
  ...
  @usableFromInline
  internal static let alphaNumericAsciiSet: NonEmptySet<UInt8> = { // TODO: tests to init this static let
    let set = mutate(value: Set<UInt8>()) {
      $0.formUnion(97...122) // lowerCaseAsciiRange
      $0.formUnion(65...90) // upperCaseAsciiRange
      $0.formUnion(48...57) // numericAsciiRange
    }
    return NonEmpty(base: set)!
  }()
  ...
  @usableFromInline
  internal static let allPrintableExcludingReservedAsciiSet: NonEmptySet<UInt8> = {
    let set = mutate(value: Set<UInt8>()) {
